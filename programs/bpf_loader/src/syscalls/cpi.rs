...
fn update_caller_account(
    ...
    *caller_account.owner = *callee_account.get_owner();
    ...
    let _realloc_perms_drop = if direct_mapping {
        if caller_account.original_data_len > 0 {
        ...
        if !is_loader_deprecated {
            // the realloc region starts at the end of the original data
            let realloc_vm_addr = caller_account
                .vm_data_addr
                .saturating_add(caller_account.original_data_len as u64);
            ...
            // for the duration of this function and then if necessary drop
            // perms back right at the end.
            realloc_region.state.set(MemoryState::Writable);

            struct ReallocDropGuard<'a> {
                realloc_region: &'a MemoryRegion,
                state: MemoryState,
            }

            impl<'a> Drop for ReallocDropGuard<'a> {
                fn drop(&mut self) {
                    self.realloc_region.state.set(self.state);
                }
            }

            Some(ReallocDropGuard {
                realloc_region,
                state: if callee_account.can_data_be_changed().is_ok() {
