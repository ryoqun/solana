...
fn execute_batches(
    ...
    Ok(())
        ...
        #[derive(Debug)]
        struct NonblockingSchedulerWithDepGraph(NonblockingScheduler<SleepyHandler2>, std::sync::Mutex<Vec<SanitizedTransaction>>, crossbeam_channel::Receiver<Signature>);
        ...
        fn bench_txes_with_long_serialized_runs(_bencher: &Bencher) {
            ...
            let lock_ignoring_scheduler = NonblockingScheduler::spawn(pool, context.clone(), 10, handler2);
            let mut lock_adhering_scheduler = NonblockingSchedulerWithDepGraph(lock_ignoring_scheduler, Default::default(), receiver);
        }
    }
}
