        ...
        ("accounts", Some(arg_matches)) => {
            ...
            match load_bank_forks(
                ...
                Ok((bank_forks, _leader_schedule_cache, _snapshot_hash)) => {
                    ...
                    println!("---");
                    let mut supplies_by_mint = BTreeMap::<_, (u64, u64, u64)>::new();
                    ...
                    for (pubkey, (account, slot)) in accounts.into_iter() {
                        ...
                        if account.owner() == &spl_token_id_v2_0() {
                            if data_len == spl_token_v2_0::state::Account::get_packed_len() {
                                if let Ok(spl_account) =
                                    spl_token_v2_0::state::Account::unpack(&account.data())
                                {
                                    let mut new = supplies_by_mint
                                        .get(&spl_account.mint)
                                        .copied()
                                        .unwrap_or_default();
                                    new.0 += 1 as u64;
                                    if spl_account.amount > 0 {
                                        new.2 += spl_account.amount;
                                    } else {
                                        new.1 += 1 as u64;
                                    }
                                ...
                                if let Ok(spl_mint) =
                                    spl_token_v2_0::state::Mint::unpack(&account.data())
                                {
                    ...
                    for (mint_address, mint) in &mints {
                        if let Some((calculated_count, empty_count, calculated_supply)) =
                            supplies_by_mint.get(
                                &spl_token_v2_0::solana_program::pubkey::Pubkey::new(
                                    &mint_address.to_bytes(),
                                ),
                            )
                        {
                            println!(
                                "mint_address: {} ({} holders (empty: {})) {} == {} => {}",
                                mint_address,
                                calculated_count,
                                empty_count,
                                mint.supply,
                                calculated_supply,
                                (if mint.supply == *calculated_supply {
                                    "OK"
                                } else {
                                    "NG"
                                })
