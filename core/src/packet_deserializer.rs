...
mod tests {
    ...
    fn test_deserialize_and_collect_packets_simple_batches() {
        ...
        let results = PacketDeserializer::deserialize_and_collect_packets(packet_count, &packet_batches.iter().map(|a| BankingPacketBatch::new((a, None))).collect::<Vec<_>>(), None);
