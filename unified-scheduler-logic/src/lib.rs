...
impl UsageQueueInner {
    ...
    fn unlock(&mut self, requested_usage: RequestedUsage) -> Option<UsageFromTask> {
        ...
        match &mut self.current_usage {
            ...
            Some((Usage::Writable, _task)) => match requested_usage {
            ...
            context.with_usage_queue_mut(&mut self.usage_queue_token, |usage_queue| {
                ...
                let lock_result = match &usage_queue.current_usage {
                    Some((current_usage, current_task)) if current_task.blocked_usage_count(&mut self.count_token) > 0 && new_task.index < current_task.index => {
                        match current_usage {
                            Usage::Writable => {
                                usage_queue.insert_blocked_usage_from_task(current_task.index, (RequestedUsage::Writable, current_task.clone()));
...
mod tests {
    ...
    fn test_unreachable_unlock_conditions3() {
        ...
        usage_queue
            .0
            .with_borrow_mut(&mut state_machine.usage_queue_token, |usage_queue| {
                usage_queue.current_usage = Some((Usage::Readonly(ShortCounter::one()), task));
                let _ = usage_queue.unlock(RequestedUsage::Writable);
            });
    }
}
