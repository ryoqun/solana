...
impl TaskInner {
    ...
    fn with_pending_mut<R>(&self, token: &mut BlockedUsageCountToken, f: impl FnOnce(&mut CounterWithStatus) -> R) {
        self.blocked_usage_count
                    ...
                    match usage_queue.try_lock(
                        ...
                        LockResult::Ok(()) => {
                            buffered_task_from_queue2.task().with_pending_mut(&mut self.count_token, |c| {
                                c.pending_usage_queue.remove(usage_queue).then_some(()).or_else(|| panic!());
                            });
    ...
    pub fn create_task(
        ...
        let lock_contexts = transaction
            ...
            .map(|(index, address)| {
                ...
                pending_usage_queue.insert(Arc::as_ptr(&usage_queue.0));
                LockContext::new(
                    usage_queue,
