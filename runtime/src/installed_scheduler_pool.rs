...
#[derive(Clone)]
pub struct BankWithScheduler(Arc<BankWithSchedulerInner>);
pub struct BankWithSchedulerInner(Arc<Bank>, InstalledSchedulerRwLock);
...
impl BankWithScheduler {
    pub(crate) fn new(bank: Arc<Bank>, scheduler: Option<InstalledSchedulerBox>) -> Self {
        Self(Arc::new(BankWithSchedulerInner(
            bank,
    ...
    pub fn scheduler(&self, callback: impl FnOnce(&InstalledSchedulerRwLock)) {
        callback(&self.0 .1)
    }

    pub fn with_scheduler(&self) -> bool {
        self.0 .1.read().unwrap().is_some()
    }
    ...
    pub fn wait_for_completed_scheduler(&self) -> Option<ResultWithTimings> {
        self.0.wait_for_scheduler(WaitReason::TerminatedToFreeze)
    }
    ...
    pub fn schedule_transaction_executions<'a>(
        ...
        let scheduler_guard = self.0 .1.read().unwrap();
...
impl BankWithSchedulerInner {
    ...
    fn wait_for_scheduler(&self, reason: WaitReason) -> Option<ResultWithTimings> {
        Self::do_wait_for_scheduler(&self.0, &self.1, reason)
    }
